...
var controller = {
			...
			(err, calificacion) => {
				...
				if (req.body.userEmisorID == undefined) {
					return res
						.status(404)
						.send({
							message: "No se encuentra el campo userEmisorID en la solicitud",
						});
				}
				if (req.body.userReceptorID == undefined) {
					return res
						...
						.send({
							message:
								"No se encuentra el campo userReceptorID en la solicitud",
						});
				}
				if (req.body.value == undefined) {
					return res
						.status(404)
						.send({
							message: "No se encuentra el campo value en la solicitud",
						});
				}
				if (req.body.userEmisorID == req.body.userReceptorID) {
					return res
						.status(404)
						.send({ message: "No puedes calificarte a ti mismo" });
				}
				if (calificacion.length > 0) {
					return res
						.status(404)
						.send({ message: "Ya calificaste a este usuario" });
				} else {
					...
					calificacion.save((err, calificacionStored) => {
						if (err)
							return res.status(500).send({ message: "Error en Servidor." });

						if (!calificacionStored)
							return res
								.status(404)
								.send({ message: "No se ha podido guardar la calificacion." });
		...
		Calificacion.findByIdAndUpdate(
			...
			(err, calificacionUpdated) => {
				...
				if (!calificacionUpdated)
					return res
						.status(404)
						.send({ message: "El id de la Calificacion no existe." });
	...
	getCalificacionesR: function (req, res) {
		...
		if (userReceptorID == null)
			return res
				.status(404)
				.send({ message: "No se encuentra el parametro userReceptorID." });
