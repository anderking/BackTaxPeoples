			...
			(err, like) => {
				...
				if (req.body.userID == undefined) {
					return res
						.status(404)
						.send({
							message: "No se encuentra el campo userID en la solicitud",
						});
				}
				if (like.length > 0) {
					return res
						.status(404)
						.send({ message: "Ya le diste like a esta publicación" });
				} else {
					var like = new Like();
					var params = req.body;
					...
					like.save((err, likeStored) => {
						...
						if (!likeStored)
							return res
								.status(404)
								.send({ message: "No se ha podido guardar el like." });
	...
	disLike: function (req, res) {
		Like.remove(
			...
			(err, like) => {
				...
				if (like.n == 0)
					return res.status(404).send({ message: "Ya fue eliminado." });
		...
		Like.find(
			...
			(err, likesPublication) => {
				...
				if (!likesPublication)
					return res
						.status(404)
						.send({ message: "No hay likes de esta publicación." });
		...
		Like.find(
			...
			(err, likesUsers) => {
				...
				if (!likesUsers)
					return res
						.status(404)
						.send({ message: "Este usuario no tiene likes." });
