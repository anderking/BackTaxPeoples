...
var controller = {
		...
		Publication.find(
			...
			(err, publication) => {
				if (err) {
					return res
						.status(500)
						.send({ message: "Error en el Servidor publication" });
				}
				...
				if (publication.length <= 0) {
					return res
						...
						.send({
							message:
								"La publicación no existe, no puedes agregar un comentario",
						});
				} else {
					User.find(
						{
						...
						(err, publication) => {
							if (err) {
								return res
									.status(500)
									.send({ message: "Error en el Servidor user" });
							}
							...
							if (publication.length <= 0) {
								return res
									...
									.send({
										message:
											"El usuario no existe, no puedes agregar un comentario",
									});
							} else {
								...
								coment.save((err, comentStored) => {
									if (err)
										return res
											.status(500)
											.send({ message: "Error en Servidor." });
	...
	getComent: function (req, res) {
		...
		Coment.findById(comentId, (err, coment) => {
			...
			if (!coment)
				return res
					.status(404)
					.send({ message: "El id del comentario no existe." });
		...
		Coment.find()
			...
			.exec((err, coments) => {
				...
				if (!coments)
					return res
						.status(404)
						.send({ message: "No hay comentarios que mostrar." });
	...
	getcomentsPublication: function (req, res) {
		...
		if (publicationID == null)
			return res
				.status(404)
				.send({ message: "No se encuentra el parametro publicationID." });

		Coment.find({ publicationID: publicationID }, (err, comentsPublication) => {
			if (err)
				return res.status(500).send({ message: "Error en el servidor." });

			if (!comentsPublication)
				return res
					.status(404)
	...
	deleteComent: function (req, res) {
		...
		Coment.findByIdAndRemove(comentId, (err, comentRemoved) => {
			...
			if (!comentRemoved)
				return res.status(404).send({ message: "La el comentario no existe." });
