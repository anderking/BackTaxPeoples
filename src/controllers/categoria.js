...
var controller = {
		...
		Categoria.find(
			...
			(err, categoria) => {
				...
				if (categoria.length > 0) {
					return res
						.status(404)
						.send({ message: "El nombre de la categoria ya está registrada" });
				} else {
					...
					categoria.save((err, categoriaStored) => {
						...
						if (!categoriaStored)
							return res
								.status(404)
								.send({ message: "No se ha podido guardar la Categoria." });
	...
	getCategoria: function (req, res) {
		...
		Categoria.findById(categoriaId, (err, categoria) => {
			...
			if (!categoria)
				return res
					.status(404)
					.send({ message: "El id de la Categoria no existe." });
			return res.status(200).send({
				categoria,
			});
		});
		...
		Categoria.find()
			...
			.exec((err, categorias) => {
				...
				if (!categorias)
					return res
						.status(404)
						.send({ message: "No hay Categorias que mostrar." });
	...
	getpublicationsCategoria: function (req, res) {
		...
		if (categoriaID == null)
			return res
				.status(404)
				.send({ message: "No se encuentra el parametro categoriaID." });

		Publication.find(
			{ categoriaID: categoriaID },
			(err, publicationsCategoria) => {
				if (err)
					return res.status(500).send({ message: "Error en el servidor." });

				if (!publicationsCategoria)
					return res
						.status(404)
					...
					Publication.find(
						...
						(err, publications) => {
							if (publications.length > 0) {
								...
								Publication.remove(
									{ categoriaID: categorias[i - 1]._id },
									(err, publicationsRemoved) => {}
								);
							}
						}
