...
var controller = {
			...
			(err, ruta) => {
				...
				if (req.body.name == undefined) {
					return res.status(404).send({
						message: "No se encuentra el campo name en el formulario",
					});
				}
				if (ruta.length > 0) {
					return res
						.status(404)
						.send({ message: "El nombre de la ruta ya está registrada" });
				} else {
					var ruta = new Ruta();
					var params = req.body;
					...
					(ruta.name =
						...
						(ruta.description =
							params.description.charAt(0).toUpperCase() +
							params.description.slice(1));
					...
					ruta.save((err, rutaStored) => {
						...
						if (!rutaStored)
							return res
								.status(404)
								.send({ message: "No se ha podido guardar la Ruta." });
	...
	getRuta: function (req, res) {
		...
		Ruta.findById(rutaId, (err, ruta) => {
			...
			if (!ruta)
				return res.status(404).send({ message: "El id de la Ruta no existe." });
	...
	getRutas: function (req, res) {
		Ruta.find()
			...
			.exec((err, rutas) => {
				...
				if (!rutas)
					return res.status(404).send({ message: "No hay Rutas que mostrar." });
	...
	getpublicationsRuta: function (req, res) {
		...
		if (rutaID == null)
			return res
				.status(404)
				.send({ message: "No se encuentra el parametro rutaID." });
		...
		Publication.find({ rutaID: rutaID }, (err, publicationsRuta) => {
			...
			if (!publicationsRuta)
				return res
					.status(404)
					.send({ message: "El id de la Publicación no existe." });
	...
	updateRuta: function (req, res) {
		...
		update.description =
			update.description.charAt(0).toUpperCase() + update.description.slice(1);
		...
		Ruta.findByIdAndUpdate(
			...
			(err, rutaUpdated) => {
				...
				if (!rutaUpdated)
					return res
						.status(404)
						.send({ message: "El id de la Ruta no existe." });
					...
					Publication.remove({ rutaID: rutaId }, (err, publicationsRemoved) => {
						if (err)
							return res.status(500).send({
								message: "No se ha podido borrar las publicaciones del usuario",
							});
					});
				}
			});
