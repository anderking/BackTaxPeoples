...
var controller = {
			...
			(error, result) => {
				...
				if (result.length <= 0) {
					return res
						...
						.send({
							message:
								"El usuario no se encuentra, no podemos crear una publicación sin un usuario asociado",
						});
				} else {
					Categoria.find(
						{
						...
						(error, result) => {
							...
							if (result.length <= 0) {
								return res
									...
									.send({
										message:
											"La Categoria no se encuentra, no podemos crear una publicación sin una categoria asociada",
									});
							} else {
								Ruta.find(
									{
									...
									(error, result) => {
										...
										if (result.length <= 0) {
											return res
												...
												.send({
													message:
														"La Ruta no se encuentra, no podemos crear una publicación sin una ruta asociada",
												});
										} else {
	...
	getPublication: function (req, res) {
		...
		if (publicationId == null)
			return res
				.status(404)
				.send({ message: "No se encuentra el parametro id." });
	...
	getPublicationsUser: function (req, res) {
		...
		if (userID == null)
			return res
				.status(404)
				.send({ message: "No se encuentra el parametro userID." });
		...
		Publication.find()
			...
			.exec((err, publications) => {
				...
				if (!publications)
					return res
						.status(404)
						.send({ message: "No hay Publicaciones que mostrar." });
		...
		Publication.findByIdAndRemove(publicationId, (err, publicationRemoved) => {
			...
			Like.remove({ publicationID: publicationId }, (err, likes) => {
				if (err)
					return res
						.status(500)
						.send({ message: "No se ha podido borrar los likes del usuario" });
			});
			...
			Coment.remove({ publicationID: publicationId }, (err, likes) => {
				if (err)
					return res
						.status(500)
						.send({
							message: "No se ha podido borrar los comentarios del usuario",
						});
			});
