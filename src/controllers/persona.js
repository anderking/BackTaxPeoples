...
var controller = {
		...
		Persona.find(
			...
			(error, result) => {
				...
				if (result.length > 0) {
					return res
						.status(404)
						.send({ message: "La cedula del Usuario ya esta registrada" });
				} else {
					Persona.find(
						{
						...
						(error, result) => {
							...
							if (result.length > 0) {
								return res
									.status(404)
									.send({
										message: "Este Usuario ya tiene una Persona asociada",
									});
							} else {
								Empresa.find(
									{
									...
									(error, result) => {
										...
										if (result.length > 0) {
											return res
												.status(404)
												.send({
													message: "Este Usuario ya tiene una Empresa asociada",
												});
										} else {
											var params = req.body;
											...
											const persona = new Persona({
												...
												estado:
													params.estado.charAt(0).toUpperCase() +
													params.estado.slice(1),
											...
											persona.save((err, personaStored) => {
												...
												if (!personaStored)
													return res
														...
														.send({
															message:
																"No se ha podido guardar la Persona asociada al Usuario",
														});

												return res.status(200).send({
													persona: personaStored,
													message:
														"Persona asociada al Usuario creada satisfactoriamente",
												});
											});
										}
									}
	...
	getPersona: function (req, res) {
		...
		if (userID == null)
			return res
				.status(404)
				.send({ message: "La Persona asociada al Usuario no existe." });
		...
		Persona.find({ userID: userID }, (err, persona) => {
			...
			if (!persona)
				return res
					.status(404)
					.send({
						message: "El id de la persona asociada al Usuario no existe.",
					});
		...
		Persona.find()
			...
			.exec((err, personas) => {
				...
				if (!personas)
					return res
						.status(404)
						.send({ message: "No hay Personas que mostrar." });
